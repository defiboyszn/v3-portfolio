...
export const Interview = () => {
  ...
  return (
    <>
      <motion.div
        id="interviews"
        className="mt-24 mb-10 overflow-hidden flex flex-col items-center font-bricolage h-full md:h-screen"
      >
        <motion.h1
          initial={{
            y: -1000,
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-11 px-5">
          {interview.map((data, i) => (
            ...
            <motion.div
              key={i}
              data-aos={animationTypes[i % animationTypes.length]}
              className="md:px-6 py-4 flex flex-col gap-y-2"
            >
              <motion.iframe
                className="w-[290px] md:w-[560px] h-[315px]"
                src={data.embed}
                ...
                allowfullscreen
              ></motion.iframe>
              <motion.div className="font-normal font-bricolage text-white w-[290px] md:w-fit text-xl mb-2">
                {data.name}
              </motion.div>
            </motion.div>
            // </Card>
